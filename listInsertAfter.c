/* listInsertAfter() -- insert new item after specified node in list */
...
/* NOTE: the 'newitem' should be malloc-ed before calling this function. */
			      ...
			      struct Node *t)  /* ptr to node in a list  */
{
  ...
  listNodeCheck(t, "l.I.A:prev");
  listNodeCheck(t->next, "l.I.A:next");
  x = listNodeCreate("l.I.A");
  x->item = newitem;
  x->next = t->next;
  t->next = x;
  return(x);
