...
#define VECTOR_VS(V) \
    double V##_0, V##_1, V##_2

#define MATRIX_MS(M) \
    double M##_00, M##_01, M##_02, \
           M##_10, M##_11, M##_12, \
	   M##_20, M##_21, M##_22
...
#define MVM_PRIVATE_VARIABLES \
	...
	VECTOR_VS(vtemp);                \
	MATRIX_MS(mtemp);                \
        double xca, xsa; MATRIX_MS(xt2); \
        double yca, ysa; MATRIX_MS(yt2); \
        double zca, zsa; MATRIX_MS(zt2); \
    } MVM
...
#define VECTOR_FILL(TV, V, v0)\
                TV(V,0) = TV(V,1) = TV(V,2) = v0;
...
#define VECTOR_LENGTH(TV, V) \
                (sqrt(VECTOR_DOT(TV, V, TV, V)))
...
#define VECTOR_ANGLE(TA, A, TB, B)                                    \
                (acos((VECTOR_DOT(TA, A, TB, B)*(1.-1e-14))           \
                    / (VECTOR_LENGTH(TA, A) * VECTOR_LENGTH(TB, B))))
...
#define MATRIX_COPY(TA, A, TB, B)  \
        TA(A,0,0)=TB(B,0,0); TA(A,0,1)=TB(B,0,1); TA(A,0,2)=TB(B,0,2); \
        TA(A,1,0)=TB(B,1,0); TA(A,1,1)=TB(B,1,1); TA(A,1,2)=TB(B,1,2); \
        TA(A,2,0)=TB(B,2,0); TA(A,2,1)=TB(B,2,1); TA(A,2,2)=TB(B,2,2); 
...
#define MATRIX_DETERMINANT(TA, A)                  \
                (TA(A,0,0) * TA(A,1,1) * TA(A,2,2) \
                -TA(A,0,0) * TA(A,1,2) * TA(A,2,1) \
                -TA(A,0,1) * TA(A,1,0) * TA(A,2,2) \
                +TA(A,0,1) * TA(A,1,2) * TA(A,2,0) \
                +TA(A,0,2) * TA(A,1,0) * TA(A,2,1) \
                -TA(A,0,2) * TA(A,1,1) * TA(A,2,0))

#define ROTATION_ANGLE(TRM, RM) \
        (acos(0.5 * (TRM(RM,0,0) + TRM(RM,1,1) + TRM(RM,2,2) - 1.0)))
...
#define MATRIX_MULT(TC,C,TA,A,TB,B)              \
     MS(MVM.mtemp,0,0)= TA(A,0,0)*TB(B,0,0)  \
		      + TA(A,0,1)*TB(B,1,0)  \
                      + TA(A,0,2)*TB(B,2,0); \
     MS(MVM.mtemp,1,0)= TA(A,1,0)*TB(B,0,0)  \
                      + TA(A,1,1)*TB(B,1,0)  \
                      + TA(A,1,2)*TB(B,2,0); \
     MS(MVM.mtemp,2,0)= TA(A,2,0)*TB(B,0,0)  \
                      + TA(A,2,1)*TB(B,1,0)  \
                      + TA(A,2,2)*TB(B,2,0); \
     MS(MVM.mtemp,0,1)= TA(A,0,0)*TB(B,0,1)  \
                      + TA(A,0,1)*TB(B,1,1)  \
                      + TA(A,0,2)*TB(B,2,1); \
     MS(MVM.mtemp,1,1)= TA(A,1,0)*TB(B,0,1)  \
                      + TA(A,1,1)*TB(B,1,1)  \
                      + TA(A,1,2)*TB(B,2,1); \
     MS(MVM.mtemp,2,1)= TA(A,2,0)*TB(B,0,1)  \
                      + TA(A,2,1)*TB(B,1,1)  \
                      + TA(A,2,2)*TB(B,2,1); \
     MS(MVM.mtemp,0,2)= TA(A,0,0)*TB(B,0,2)  \
                      + TA(A,0,1)*TB(B,1,2)  \
                      + TA(A,0,2)*TB(B,2,2); \
     MS(MVM.mtemp,1,2)= TA(A,1,0)*TB(B,0,2)  \
                      + TA(A,1,1)*TB(B,1,2)  \
                      + TA(A,1,2)*TB(B,2,2); \
     MS(MVM.mtemp,2,2)= TA(A,2,0)*TB(B,0,2)  \
                      + TA(A,2,1)*TB(B,1,2)  \
                      + TA(A,2,2)*TB(B,2,2); \
...
#define ABOUT_X(TRM,RM,A)                     \
        MVM.xca = cos(A);                     \
        MVM.xsa = sin(A);                     \
	MATRIX_MULT_9(MS,MVM.xt2, TRM,RM,     \
		    1.,     0.,     0.,       \
		    0., MVM.xca,-MVM.xsa,     \
		    0.,+MVM.xsa, MVM.xca)     \
...
#define ABOUT_Y(TRM,RM,A)                     \
        MVM.yca = cos(A);                     \
        MVM.ysa = sin(A);                     \
	MATRIX_MULT_9(MS,MVM.yt2, TRM,RM,     \
		     MVM.yca,   0., -MVM.ysa, \
		         0.,    1.,      0.,  \
		    +MVM.ysa,   0., MVM.yca)  \
...
#define ABOUT_Z(TRM,RM,A)                    \
        MVM.zca = cos(A);                    \
        MVM.zsa = sin(A);                    \
	MATRIX_MULT_9(MS,MVM.zt2, TRM,RM,    \
		     MVM.zca, -MVM.zsa,  0., \
		    +MVM.zsa,  MVM.zca,  0., \
		         0.,      0.,    1.) \
...
#define EULER_2_MATRIX(TAA, AA, TE, E)                              \
        ANGLES_2_MATRIX(TAA, AA, Z,TE(E,0), X,TE(E,1), Z,TE(E,2), \
                                                        NOP,NOP, NOP,NOP);
...
#define SMALL_2_MATRIX(TRM, RM, TE, E)                                  \
    ANGLES_2_MATRIX(TRM, RM, X, TE(E,0), Y, TE(E,1), Z, TE(E, 2), \
                                                NOP, NOP, NOP, NOP)
...
#define MATRIX_TRANSPOSE(TMT, MT, TM, M) \
        TMT(MT,0,0)=TM(M,0,0); TMT(MT,0,1)=TM(M,1,0); TMT(MT,0,2)=TM(M,2,0); \
        TMT(MT,1,0)=TM(M,0,1); TMT(MT,1,1)=TM(M,1,1); TMT(MT,1,2)=TM(M,2,1); \
        TMT(MT,2,0)=TM(M,0,2); TMT(MT,2,1)=TM(M,1,2); TMT(MT,2,2)=TM(M,2,2);
