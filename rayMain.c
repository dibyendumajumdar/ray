...
{
    char
	str[LONG], line[LONG], cmd[SHORT], c2[SHORT], wave_type[SHORT],
	assign_by[SHORT], name[LONG], vignette[SHORT], temp[LONG],
	filename[SHORT], form[LONG], print_mode[LONG], psname[SHORT],
	pltmode[LONG], set_name[LONG];  
    char *p, *p_set_name=NULL;
    double
	xyz[3], XYZ[3], radius, step, angle, db, c, e, a2, a4, mu1,
	v[3], V[3], tol=0.00001, hcm, wcm, wps, to[3];
    int
	nv, nve, steps, num, code1, code2, d=4, i, j, k=65, axis_mask;
	...
	if (line[0] == '#') {                /* is it a comment line? */
		...
		if (strcmp(cmd, "Quit") == 0) {
		    ...
		    strcpy(form,
			   "%s %s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf");
		    strcat(form, " %s %lf %lf %lf %lf %lf %lf %lf");
		    if ((nv = sscanf(line, form,
				     c2, name, &c, &e, &a2, &a4, &mu1,
				     &xyz[0], &xyz[1], &xyz[2],
				     &XYZ[0], &XYZ[1], &XYZ[2],
				     vignette,
				     &v[0], &v[1], &v[2],
		    ...
		    rayAddSurface(System, name, c, e, a2, a4, mu1,
				  xyz, XYZ, vign_type, v, V, radius);
		    ...
		    if ((nv = sscanf(line, "%s %s", c2, set_name)
			 ) != (nve = 2)) {
		    ...
		    if ((nv = sscanf(line, form,
				     c2, wave_type, &xyz[0], &xyz[1], &xyz[2],
				     &XYZ[0], &XYZ[1], &XYZ[2],
				     &radius, &step, &steps, &axis_mask, &num,
				     &angle, &db,
				     &code1, &code2, assign_by)
			 ) != (nve = 18)) {
			printf("Expected %d values, got nv=%d -->ABORT\n",
			       nve, nv);
			exit(EXIT_FAILURE);
		    ...
		    rayGenerator(BundleSet, p_set_name, wave_type, xyz, XYZ,
				 radius, step, steps, axis_mask, num,
				 angle, db, code1, code2,
				 a_b);
		    ...
		    if ((nv = sscanf(line, "%s %lf %lf %lf %s",
				     c2, &xyz[0], &xyz[1], &radius,
				     print_mode)) != 5) {
		    ...
		    if ((nv = sscanf(line, "%s %lf %lf %lf %lf %lf %lf %s %s",
				     c2, &hcm, &wcm, &wps,
				     &to[0], &to[1], &to[2],
				     pltmode, psname)) != (nve = 9)) {
			printf("Expected %d values, got nv=%d -->ABORT\n",
			       nve, nv);
			exit(EXIT_FAILURE);
		    }
