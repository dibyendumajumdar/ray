		   ...
		   int d)                    /* digits after decimal point */
{
    ...
    struct Plane *p;
    char
	head[FORMMAX], body[FORMMAX], temp[FORMMAX], form[FORMMAX],
	*aberrations[AMNIMAX], temp2[FORMMAX], *match;
    double small, val;
    int i, j, k, w;
    struct {char *original; char *replace;} zeros[] = {
	{"-0.", " -."}, {" 0.", "  ."}};
