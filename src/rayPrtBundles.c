		   ...
		   int d)	     /* digits after decimal point */
{
	...
	while (s->next->next != s->next) {
		...
		while (t->next->next != t->next) {
			...
			i++;
			if (i == 1) {
				sprintf(head_format, "%%2s%%%ds%%%ds%%%ds %%%ds%%%ds%%%ds%%%ds%%7s%%4s\n", w, w, w, dc,
					dc, dc, w);
				/* printf("rayPrtBundles: head=<%s>\n", head_format); */
				sprintf(
				    body_format,
				    "%%2d%%%d.%dlf%%%d.%dlf%%%d.%dlf %%%d.5lf%%%d.5lf%%%d.5lf%%%d.%dlf%%7.4lf%%4d\n", w,
				    d, w, d, w, d, dc, dc, dc, w, d);
				/* printf("rayPrtBundles: body=<%s>\n", body_format); */
				printf(head_format, "i", "T[0] ", "T[1] ", "T[2] ", "Q[0] ", "Q[1] ", "Q[2] ",
				       "Length ", "Inten", " Color");
				printf(head_format, "--", "-------", "-------", "-------", "-------", "-------",
				       "-------", "-------", "------", "---");
			}
			printf(body_format, i, r->T[0], r->T[1], r->T[2], r->Q[0], r->Q[1], r->Q[2], r->Length,
			       r->Intensity, r->ColorCode);
		}
		if (i == 0) {
	...
	if (b == 0)
		printf("rayPrtBundles: set <%s> contains no bundles of rays.\n\n", (char *)set->item);
}
