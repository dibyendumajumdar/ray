	...
	if (strcmp(wave_type, "spherical") == 0) { /* make diverging cones of rays: */
				...
				for (k = -case_steps * sk; k <= +case_steps * sk; k++) {
					...
					if (rad <= ((case_step * case_steps) + tiny)) {
						if (bundle_name != NULL)
							...
							sprintf(temp, "%.4g %.4g %.4g", xyz[0], xyz[1], xyz[2]);
						...
						for (ir = -ray_steps; ir <= +ray_steps; ir++) {
							dl[0] = 0.0;
							dl[1] = ir * (wave_radius / ((ray_steps == 0) ? 1 : ray_steps));
							for (jr = -ray_steps; jr <= +ray_steps; jr++) {
								...
								euler[0] = 0.0;
								euler[1] = -atan2(wave_direction[2], wave_direction[0]);
								euler[2] = -atan2(wave_direction[1], wave_direction[0]);
								VNR_2_MATRIX(a, euler);
								/* cone->wave_direction: */
								MATRIX_VECTOR_MULT(VC, r->Q, MC, a, VC, XYZ);
								r->Length = 0.0;
								r->Intensity =
								    pow(cos((rb / wave_radius) * taper_angle), 2.0 * q);
		...
		for (i = -case_steps * si; i <= +case_steps * si; i++) {
			...
			for (j = -case_steps * sj; j <= +case_steps * sj; j++) {
				...
				if (sqrt(XYZ[1] * XYZ[1] + XYZ[2] * XYZ[2]) < ((case_step * case_steps) + tiny)) {
					...
					dl[0] = 0.0;
					for (ir = -ray_steps; ir <= +ray_steps; ir++) {
						dl[1] = ir * (wave_radius / (ray_steps ? ray_steps : 1));
						for (jr = -ray_steps; jr <= +ray_steps; jr++) {
							dl[2] = jr * (wave_radius / (ray_steps ? ray_steps : 1));
							if ((rb = sqrt(dl[1] * dl[1] + dl[2] * dl[2])) >
							    (wave_radius + tiny))
								continue;
							r = (struct Ray *)listAlloc(1, sizeof(struct Ray));
