		...
		int d)			/* digits after decimal point */
{
	...
	while (bundle->next->next != bundle->next) {
		...
		if (i == 1) {
			w = d + 5;
			sprintf(head, "%%3s%%%ds%%4s%%%ds%%%ds%%%ds%%%ds", NAMEMAX, w, w, w, w);
			sprintf(body, "%%3d%%%ds%%4d%%%d.%dlf%%%d.%dlf%%%d.%dlf%%%d.%dlf", NAMEMAX, w, d, w, d, w, d, w,
				d);
			w = d + 3;
			...
			strcat(body, temp);
			printf(head, "i", "bundle_name", "n", "xc", "yc", "zc", "lc", "xs", "ys", "zs", "ls");
			printf(head, "--", "----------------", "---", "------", "------", "------", "-------", "-----",
			       "-----", "-----", "-----");
		}
		printf(body, i, f->name, f->n, f->xyz[0], f->xyz[1], f->xyz[2], f->lc, f->xyzs[0], f->xyzs[1],
		       f->xyzs[2], f->ls);
	}
	if (i == 0) {
