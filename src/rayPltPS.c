...
#define XY2P(V, i, j) (S2P((V[i]) - to_point[i], C2P(width_cm))), (S2P((V[j]) - to_point[j], C2P(height_cm)))
	     ...
	     char psname[])	    /* Postscript output file        */
{
	...
	time(&now);
	now_tm = gmtime(&now);
	fprintf(fp, "%%%%CreationDate: UTC %s", asctime(now_tm));
	fprintf(fp, "%%%%Title: %s\n", psname);
	...
	while (s->next->next != s->next) { /* loop over list of segments */
		...
		if (Seg->ColorCode % 1000 != prev_color) {
			...
			while (h->next->next != h->next) { /* search for ColorCode */
				h = h->next;
				if (((struct HueTable *)h->item)->ColorCode == Seg->ColorCode % 1000) {
		...
		for (i = 0, diff = 0.0; i < 3; i++)
			diff += fabs(Seg->T1[i] - prev_T[i]);
		jx = 2;
		jy = 0;
		jx = 0;
		jy = 1;
		if (diff != 0.0)
			fprintf(fp, "%8.1f %8.1f moveto\n", XY2P(Seg->T1, jx, jy));
	...
	sprintf(temp, "; width=%.2f at [%.1f,%.1f]", width, to_point[0], to_point[1]);
	strcat(header, temp);
