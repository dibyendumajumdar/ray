			...
			struct Node *Segments)     /* appends to Segments   */
{
    ...
    double
	r[4], left[4], right, weight, c[4][4],
	o_minus_c, x[4], Isum, size, sphere, radius;
    union ad1d2 {
	double d1[16];
	double d2[4][4];
    } a;
	...
	if ((focus = (struct Focus *)listAlloc(1, sizeof(struct Focus))) 
	    == NULL) {
	...
	for (i = 0; i < 4; i++)                   /* copy to lower triangular */
	    for (j = i; j < 4; j++)
		a.d2[j][i] = a.d2[i][j];
	if (debug) printf("# rayGetFoci correlation matrix c[4][4]:\n");
	for (i = 0; i < 4; i++) {                /* compute correlation matrix */
	    if (debug) printf ("#        ");
	    for (j = 0; j < 4; j++) {
		c[i][j] = a.d2[i][j] / sqrt(a.d2[i][i] * a.d2[j][j]);
		if (debug) printf(" %6.3f", c[i][j]);
	    }
	    if (debug) printf("\n");
	}
	...
	size = sqrt(size / 3.0);
	if (size < tolerance) size = tolerance;
	...
	if (debug) printf("# rayGetFoci: sphere = %8.4f, focus->ls=%8.3g\n",
	       sphere, focus->ls);
	...
	for (k = 0; k < 3; k++) {
	    ...
	    for (i = -m; i <= +m; i+=2*m) {
		for (j = -m; j <= +m; j+=2*m) {
		    if ((seg = (struct Segment *) listAlloc(1, sizeof(struct Segment)))
			== NULL) {
