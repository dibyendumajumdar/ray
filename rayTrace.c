		       ...
		       struct Node *Segments)     /* init this list! appends */
{
    ...
    j = strlen(finalname);
    k = strlen(traced);
    if ((j + k) < NAMEMAX) strcat(finalname, traced);
    FinalBundleSet = listInitialize(finalname);

    if (strlen(Segments->item) == 0) {
	segname = (char *)listAlloc(strlen(System->item) +
				    strlen(finalname) +
				    5, sizeof(char));
    ...
    n_bundles = n_NaN_bundles = n_rays = n_NaN_rays = n_NaN_a =
	n_NaN_b = n_NaN_c = 0;
	...
	while (bundle->next->next != bundle->next) {
		...
		M_1_2 = VECTOR_DOT(VC, R->T, VC, R->T) - (e * e) +
		    (t * t) - (2.0 * t * R->T[0]);
		r_1_2 = 1. / (S->c_1*S->c_1);
		if (M_1_2 > r_1_2) M_1_2 = r_1_2;   /* SPECIAL RULE! 96-01-22 */
		xi_1 = sqrt ((R->Q[0] * R->Q[0]) -
			     S->c_1 * (S->c_1 * M_1_2 - 2.0 * M_1x));
		...
		for (j=0; j<3; j++) delta_length[j] = -R->T[j];
		...
		do {
		    ...
		    G_0 = N[0] * (x_bar_0 - R->T[0])
		                              / VECTOR_DOT(VC, R->Q, VC, N);
		...
		xi_1_p = sqrt (O_2 * (1.0 - S->mu_1 * S->mu_1) +
			       S->mu_1 * S->mu_1 * xi_1 * xi_1);
		if (isnan(xi_1_p)) {
		    ray_reject = bundle_reject = TRUE;
		    n_NaN_rays++;
		    n_NaN_c++;
	...
	if (bundle_reject) n_NaN_bundles++;
    }
    if (n_NaN_bundles != 0) {
	printf("# rayTrace: %d NaN_rays (of %d) seen in %d of %d bundles! (%d,%d,%d)\n",
	       n_NaN_rays, n_rays, n_NaN_bundles, n_bundles,
	       n_NaN_a, n_NaN_b, n_NaN_c);
    }
